...
class Particle {
	...
	public : 
		void Initialization();
		void Brownian_Geometric(int N, double dt); // Generating Brownian motion from current time step to next time step
...
void Particle::Brownian_Geometric(int N, double dt) {
	...
	//Generating Geometric Brownian Motion exp(Bt - t/2)
...
# Read the CSV file
data_particle = pd.read_csv('1D trajectory of particles (Brownian Geometric).csv')
data_mean = pd.read_csv('1D trajectory of particles (Brownian Geometric) Mean.csv')
data_variance = pd.read_csv('1D trajectory of particles (Brownian Geometric) Variance.csv')
data_covariance = pd.read_csv('1D trajectory of particles (Brownian Geometric) Covariance.csv')
data_correlation = pd.read_csv('1D trajectory of particles (Brownian Geometric) Correlation.csv')
...
plt.xlabel('Time')
plt.ylabel('Position')
plt.title('Brownian Geometric 1D Trajectories')
plt.grid()
plt.show(block=False)  

x = np.zeros(data_mean.shape[0]) + 1
plt.figure(2)
plt.plot(time_mean, data_mean['mean'])
plt.plot(time_mean, x, 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Mean')
plt.title('Brownian Geometric 1D Trajectories (Mean)')
plt.grid()
plt.show(block=False)  

plt.figure(3)
plt.plot(time_variance, data_variance['variance'])
plt.plot(time_variance, np.exp(time_variance)-1, 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Variance')
plt.title('Brownian Geometric 1D Trajectories (Variance)')
plt.grid()
plt.show(block=False)  
...
plt.title("Heatmap of Covariance Matrix for Brownian Geometric")
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
plt.colorbar(label='Value')
plt.title("Heatmap of Correlation Matrix for Brownian Geometric")
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
# Define the analytical covariance for Brownian Geometric
def analytical_covariance(s, t, T=1.0):
    ...
    covariance = np.exp(np.minimum.outer(s, t))-1
    return covariance
...
int main() {	
    ...
    for(int i = 0; i<num_particle; i++) {
    	if(num_particle > 100) {
    		if(i%t_out == 0) {
		    	...
		    	//Brownian motion
		    	cout<<"Simulating : ";
		    	particles[i].Brownian_Geometric(N, dt);
				cout<<"Done\n\n";
			}
			else {  	
		    	//Initialization
	...
	cout<<"- Trajectories : ";
	string name = "1D trajectory of particles (Brownian Geometric).csv"; OutputCSV_1D(name, dt, particles, N);   
	cout<<"Done\n"; 	
	
	cout<<"- Mean : ";
	name = "1D trajectory of particles (Brownian Geometric) Mean.csv"; OutputCSV_StatQuan1D(name, dt, mean, N, "mean");
	cout<<"Done\n";
	
	cout<<"- Variance : ";
	name = "1D trajectory of particles (Brownian Geometric) Variance.csv"; OutputCSV_StatQuan1D(name, dt, variance, N, "variance");
	cout<<"Done\n";
	
	cout<<"- Covariance : ";
	name = "1D trajectory of particles (Brownian Geometric) Covariance.csv"; OutputCSV_StatQuan2D(name, dt, covariance, N, "Cov");
	cout<<"Done\n";
	
	cout<<"- Correlation : ";
	name = "1D trajectory of particles (Brownian Geometric) Correlation.csv"; OutputCSV_StatQuan2D(name, dt, correlation, N, "Corr");
    ...
    /*
    Type this in CMD
    g++ "Geometric Brownian Motion.cpp" -IC:\Python313\include -LC:\Python313\libs -lpython313 -o "Geometric Brownian Motion" -Wl,--enable-auto-import
	"Geometric Brownian Motion.exe"
    */
}