...
class Particle {
	...
	public : 
		void Initialization();
		void Compensated_Poisson(int N, double dt, double lambda); // Generating Compensated Poisson from current time step to next time step
...
void Particle::Compensated_Poisson(int N, double dt, double lambda) {
	//Generating Brownian Motion
	double mean = 0, std = sqrt(dt);
	
	//Generating Poisson distribution
	poisson_distribution<> poisson_dist(lambda*dt);
	for(int j = 1; j<N; j++) {
		//Sampling only 1 random number for Brownian motion : 
		double dx_current = poisson_dist(gen);
	...
	//Generating compensated Poisson process Nt - t
...
string to_string_with_precision(double value, int decimals) {
    ostringstream out;
...
void call_Python(double lambda) {
...
# Define lambda value passed from C++
lambda_value = {lambda_placeholder}

# Read the CSV file
data_particle = pd.read_csv('1D trajectory of particles (Compensated Poisson).csv')
data_mean = pd.read_csv('1D trajectory of particles (Compensated Poisson) Mean.csv')
data_variance = pd.read_csv('1D trajectory of particles (Compensated Poisson) Variance.csv')
data_covariance = pd.read_csv('1D trajectory of particles (Compensated Poisson) Covariance.csv')
data_correlation = pd.read_csv('1D trajectory of particles (Compensated Poisson) Correlation.csv')
...
plt.xlabel('Time')
plt.ylabel('Position')
plt.title('Compensated Poisson 1D Trajectories')
plt.grid()
plt.show(block=False)  

x = (lambda_value-1)*time_mean
plt.figure(2)
plt.plot(time_mean, data_mean['mean'])
plt.plot(time_mean, x, 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Mean')
plt.title('Compensated Poisson 1D Trajectories (Mean)')
plt.grid()
plt.show(block=False)  

plt.figure(3)
plt.plot(time_variance, data_variance['variance'])
plt.plot(time_variance, (lambda_value)*time_variance, 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Variance')
plt.title('Compensated Poisson 1D Trajectories (Variance)')
plt.grid()
plt.show(block=False)  
...
plt.title("Heatmap of Covariance Matrix for Compensated Poisson")
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
plt.colorbar(label='Value')
plt.title("Heatmap of Correlation Matrix for Compensated Poisson")
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
# Define the analytical covariance for Compensated Poisson
def analytical_covariance(s, t, T=1.0):
    ...
    covariance = np.minimum.outer(s, t)*lambda_value
    return covariance
...
input("Press Enter to close all plots...")
    ...
    // Replace the placeholder with the actual lambda value
    size_t pos = python_code.find("{lambda_placeholder}");
    if (pos != string::npos) {
        python_code.replace(pos, string("{lambda_placeholder}").length(), to_string_with_precision(lambda, 2));
    } else {
        cerr << "Placeholder not found in Python code!" << endl;
    }
...
int main() {	
    ...
    for(int i = 0; i<num_particle; i++) {
    	if(num_particle > 100) {
    		if(i%t_out == 0) {
		    	...
		    	//Brownian motion
		    	cout<<"Simulating : ";
		    	particles[i].Compensated_Poisson(N, dt, lambda);
				cout<<"Done\n\n";
			}
			else {  	
		    	//Initialization
	...
	cout<<"- Trajectories : ";
	string name = "1D trajectory of particles (Compensated Poisson).csv"; OutputCSV_1D(name, dt, particles, N);   
	cout<<"Done\n"; 	
	
	cout<<"- Mean : ";
	name = "1D trajectory of particles (Compensated Poisson) Mean.csv"; OutputCSV_StatQuan1D(name, dt, mean, N, "mean");
	cout<<"Done\n";
	
	cout<<"- Variance : ";
	name = "1D trajectory of particles (Compensated Poisson) Variance.csv"; OutputCSV_StatQuan1D(name, dt, variance, N, "variance");
	cout<<"Done\n";
	
	cout<<"- Covariance : ";
	name = "1D trajectory of particles (Compensated Poisson) Covariance.csv"; OutputCSV_StatQuan2D(name, dt, covariance, N, "Cov");
	cout<<"Done\n";
	
	cout<<"- Correlation : ";
	name = "1D trajectory of particles (Compensated Poisson) Correlation.csv"; OutputCSV_StatQuan2D(name, dt, correlation, N, "Corr");
    ...
    /*
    Type this in CMD
    g++ "Compensated Poisson Process.cpp" -IC:\Python313\include -LC:\Python313\libs -lpython313 -o "Compensated Poisson Process" -Wl,--enable-auto-import
	"Compensated Poisson Process.exe"
    */
}