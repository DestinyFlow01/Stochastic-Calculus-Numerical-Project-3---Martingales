...
class Particle {
	...
	public : 
		void Initialization();
		void Brownian_Martingale(int N, double dt); // Generating Brownian motion from current time step to next time step
...
void Particle::Brownian_Martingale(int N, double dt) {
	...
	//Generating Brownian Martingale Bt^2-t
...
# Read the CSV file
data_particle = pd.read_csv('1D trajectory of particles (Brownian Martingale).csv')
data_mean = pd.read_csv('1D trajectory of particles (Brownian Martingale) Mean.csv')
data_variance = pd.read_csv('1D trajectory of particles (Brownian Martingale) Variance.csv')
data_covariance = pd.read_csv('1D trajectory of particles (Brownian Martingale) Covariance.csv')
data_correlation = pd.read_csv('1D trajectory of particles (Brownian Martingale) Correlation.csv')
...
plt.xlabel('Time')
plt.ylabel('Position')
plt.title('Brownian Martingale Bt^2-t 1D Trajectories')
plt.grid()
...
plt.xlabel('Time')
plt.ylabel('Mean')
plt.title('Brownian Martingale Bt^2-t 1D Trajectories (Mean)')
plt.grid()
plt.show(block=False)  

plt.figure(3)
plt.plot(time_variance, data_variance['variance'])
plt.plot(time_variance, 2*time_variance**2, 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Variance')
plt.title('Brownian Martingale Bt^2-t 1D Trajectories (Variance)')
plt.grid()
plt.show(block=False)  
...
plt.title("Heatmap of Covariance Matrix for Brownian Martingale Bt^2-t")
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
plt.colorbar(label='Value')
plt.title("Heatmap of Correlation Matrix for Brownian Martingale Bt^2-t")
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
# Define the analytical covariance for Brownian Martingale
def analytical_covariance(s, t, T=1.0):
    ...
    covariance = 2*np.minimum.outer(s, t)**2
    return covariance
...
int main() {	
    ...
    for(int i = 0; i<num_particle; i++) {
    	if(num_particle > 100) {
    		if(i%t_out == 0) {
		    	...
		    	//Brownian motion
		    	cout<<"Simulating : ";
		    	particles[i].Brownian_Martingale(N, dt);
				cout<<"Done\n\n";
			}
			else {  	
		    	//Initialization
	...
	cout<<"- Trajectories : ";
	string name = "1D trajectory of particles (Brownian Martingale).csv"; OutputCSV_1D(name, dt, particles, N);   
	cout<<"Done\n"; 	
	
	cout<<"- Mean : ";
	name = "1D trajectory of particles (Brownian Martingale) Mean.csv"; OutputCSV_StatQuan1D(name, dt, mean, N, "mean");
	cout<<"Done\n";
	
	cout<<"- Variance : ";
	name = "1D trajectory of particles (Brownian Martingale) Variance.csv"; OutputCSV_StatQuan1D(name, dt, variance, N, "variance");
	cout<<"Done\n";
	
	cout<<"- Covariance : ";
	name = "1D trajectory of particles (Brownian Martingale) Covariance.csv"; OutputCSV_StatQuan2D(name, dt, covariance, N, "Cov");
	cout<<"Done\n";
	
	cout<<"- Correlation : ";
	name = "1D trajectory of particles (Brownian Martingale) Correlation.csv"; OutputCSV_StatQuan2D(name, dt, correlation, N, "Corr");
    ...
    /*
    Type this in CMD
    g++ "Martingale Bt2-t.cpp" -IC:\Python313\include -LC:\Python313\libs -lpython313 -o "Martingale Bt2-t" -Wl,--enable-auto-import
	"Martingale Bt2-t.exe"
    */
}